...
class EnrichmentFramework():
    def __init__(self, gen_repository: str, model_gen:str, sae_name: str, layer: str, emb_repository: str, openrouter_key: str,
        ...
        self.generator_high = Generator(
            model_gen, openrouter_key, temperature=high_temperature)
        self.generator_low = Generator(
            model_gen, openrouter_key, temperature=low_temperature)
    ...
    def __call__(self, question: str, entropy_threshold: float = 1.0, density_threshold: float = 0.1, sparsity_threshold: float = 0.7) -> str:
        ...
        logging.info(f'Initial semantic entropy: {first_entropy:.3f}')

        loop_count = 0
        while loop_count < 3:
            if final_entropy <= entropy_threshold:
                logging.info("Entropy threshold met. Stopping enrichment.")
                logging.info('----------------------------------------')
                break
            ...
            logging.info(f'New semantic entropy: {final_entropy:.3f}')
            logging.info('----------------------------------------')
        logging.info('----------------------------------------')
...
if __name__ == '__main__':
    ...
    enriched_df = pd.DataFrame(results, columns=[
                            ...
                            'diff_sets', 'sparsity', 'first_entropy', 'final_entropy', 'loop_count'])
    enriched_df.to_csv('results/truthfulqa_full_enriched_questions.csv', index=False)