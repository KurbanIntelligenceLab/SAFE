...
haluscope_scorer = checker.HaloCheck(device='cuda', granularity='sentence', nli_model='mnli')  # Change to 'cuda' if a GPU is available

def haluscope(samples):
    return haluscope_scorer.score(samples)
...
class EnrichmentFramework():
    ...
    def __call__(self, question: str, entropy_threshold: float = 1.0, density_threshold: float = 0.1, sparsity_threshold: float = 0.7) -> str:
        ...
        first_entropy = haluscope(answers)
        final_entropy = first_entropy
        logging.info(f'Initial haluscope score: {first_entropy:.3f}')

        loop_count = 0
        while loop_count < 3:
            if final_entropy >= entropy_threshold:
                logging.info("haluscope score threshold met. Stopping enrichment.")
                logging.info('----------------------------------------')
                break
            ...
            # cluster_assignments = self.clustering_answers(question, answers)
            final_entropy = haluscope(answers)
            # final_entropy = semantic_entropy(cluster_assignments)
            logging.info(f'New haluscope entropy: {final_entropy:.3f}')
            logging.info('----------------------------------------')
        logging.info('----------------------------------------')
...
if __name__ == '__main__':
    ...
    for _, row in tqdm(df.head(400).iterrows()):
        question = row['input']
        try:
            results.append(enricher(question, entropy_threshold=0,
    ...
    enriched_df.to_csv('results/medai_llama_enriched_HALUSCOPE.csv', index=False)